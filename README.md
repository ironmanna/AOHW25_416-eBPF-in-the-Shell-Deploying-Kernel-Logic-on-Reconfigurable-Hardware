...
To build and deploy the system, ensure the following are installed:
...
If a port is not connected to other ports of the pipeline (e.g. mae2p_kernel0 or p2vnr_kernel0), right click on it and press "Make it external". This process is very long and prone to errors so there is an helper script inside the scripts folder named `get_connections.py` which, given the `vitis_opts.ini` file, outputs a list of tcl commands that performs all the links automatically. Users still need to make the unconnected ports as external and connect the clock and reset ports for all the IPs, but this can also be automated inside Vivado.

At this point, you should have a block design that looks like this:
...
To test the system, we used the following setup:
